...
```{r load-data}
# hfi <- read_tsv("https://www.openintro.org/data/tab-delimited/hfi.txt", progress = TRUE) breaks on posit workbench due to some timeout/firewall
hfi_2016 <- read_csv("../hfi.csv") |>
  filter(year == 2016)
```
...
```{r}
summary(hfi_2016[, c("pf_score", "pf_expression_control", "pf_movement_foreign", "pf_rol_criminal", "pf_rol")])
```
...
hfi_2016 %>% 
  select(pf_score, pf_expression_control, pf_rol) %>% 
  ggpairs()
```
...
mlr_mod <- lm_spec %>% 
fit(pf_score ~ pf_expression_control + pf_rol, data = hfi_2016)
...
$$
\hat{y} = 3.44 + 0.37 \times x_1 + 0.33 \times x_2
$$
...
When pf_expression_control is held constant, as pf_rol increases by 1 we expect pf_score to increase by 0.33.
...
plot3d(hfi_2016$pf_rol, hfi_2016$pf_expression_control, hfi_2016$pf_score, col = "blue", size = 3)
```
